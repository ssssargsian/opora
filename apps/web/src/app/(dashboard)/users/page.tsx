...
import { MailPlus, UsersRound, UserPlus } from "lucide-react";
  ...
  return (
    <>
      <section>
        ...
        {users.data && users.data.length > 0 && <div className="data-panel table-scroll"><table><thead><tr><th>ФИО</th><th>Email</th><th>Роль</th><th>Статус</th><th aria-label="Действия" /></tr></thead><tbody>{users.data.map((specialist) => <tr key={specialist.id}><td className="name-cell"><strong>{specialist.displayName}</strong></td><td className="muted-cell">{specialist.email}</td><td><span className="badge">{specialist.roleName}</span></td><td><span className={`status status-${specialist.status}`}>{statusLabel[specialist.status]}</span></td><td>{specialist.status === "invited" && canCreate && <Button size="sm" variant="ghost" disabled={resend.isPending} onClick={() => resend.mutate(specialist.id)}><MailPlus size={15} />Отправить снова</Button>}</td></tr>)}</tbody></table></div>}
